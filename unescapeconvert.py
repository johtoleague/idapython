    ...
    while i < len(inbuff):
        if inbuff[i] == '%':
            if ((i+6) <= len(inbuff)) and (inbuff[i+1] == 'u'):
                ...
                # order the bytes in big-endian format
                outArr += bytes([nextchar]) + bytes([currchar])
                i += 6
        else:
            # nothing to change
            outArr += inbuff[i].encode()
            i += 1

    return outArr


#payload = "%e589%ec81%017c%0000%6ee8%0001" 
